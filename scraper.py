...
def _extract_datetime(item):
    datetime = item.find(class_='_26yo')
    return datetime and datetime.text


def _extract_post_text(item):
    post_text = item.find(class_='_5rgt _5nk5 _5msi')
    return post_text and post_text.text
...
def _extract_html(bs_data):
    ...
    k = bs_data.find_all('article', class_='_55wo')
    #k = [item for item in bs_data.find_all() if "data-pagelet" in item.attrs]
    postBigDict = list()

    for item in k:
        postDict = dict()
        postDict['datetime'] = _extract_datetime(item)
